...
/// Call to {0} with data.len() not a multiple of {1}
#[derive(Debug, thiserror::Error, docsplay::Display)]
pub struct InvalidDataLengthError(&'static str, usize);
...
pub trait MemoryInterface {
    ...
    fn read_mem_64bit(&mut self, address: u64, data: &mut [u8]) -> Result<(), Self::Error> {
        ...
        if data.len() % 8 != 0 {
            return Err(InvalidDataLengthError("read_mem_64bit", 8).into());
        }
    ...
    fn read_mem_32bit(&mut self, address: u64, data: &mut [u8]) -> Result<(), Self::Error> {
        ...
        // conversions. Must be overridden for big endian targets.
        if data.len() % 4 != 0 {
            return Err(InvalidDataLengthError("read_mem_32bit", 4).into());
        }
    ...
    fn write_mem_64bit(&mut self, address: u64, data: &[u8]) -> Result<(), Self::Error> {
        ...
        // conversions. Must be overridden for big endian targets.
        if data.len() % 8 != 0 {
            return Err(InvalidDataLengthError("write_mem_64bit", 8).into());
        }
        let mut buffer = vec![0u64; data.len() / 8];
    ...
    fn write_mem_32bit(&mut self, address: u64, data: &[u8]) -> Result<(), Self::Error> {
        ...
        // conversions. Must be overridden for big endian targets.
        if data.len() % 4 != 0 {
            return Err(InvalidDataLengthError("write_mem_32bit", 4).into());
        }
        let mut buffer = vec![0u32; data.len() / 4];
    ...
    fn write(&mut self, mut address: u64, mut data: &[u8]) -> Result<(), Self::Error> {
        ...
        if address % 4 != 0 || len % 4 != 0 {
            ...
            if !self.supports_8bit_transfers()? {
                return Err(Into::into(MemoryNotAlignedError {
                    address,
                    alignment: 4,
                }));
            }
