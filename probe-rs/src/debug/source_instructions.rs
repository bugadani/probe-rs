...
impl VerifiedBreakpoint {
    ...
    /// 1. Filter  [`UnitInfo`], by using [`LineProgramHeader`][gimli::LineProgramHeader] to match units that include
    ///    the requested path.
    /// 2. For each matching compilation unit, get the [`LineProgram`][gimli::LineProgram] and
