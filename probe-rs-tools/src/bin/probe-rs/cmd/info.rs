...
use probe_rs::{
    architecture::arm::dp::{DebugPortVersion, MinDpSupport, DLPIDR, TARGETID},
    probe::WireProtocol,
};
use termtree::Tree;
...
use crate::{
    cmd::remote::{
        functions::info::{ApInfo, ComponentTreeNode, DebugPortInfo, DebugPortInfoNode, InfoEvent},
        ClientInterface,
    },
...
impl Cmd {
    pub async fn run(self, mut iface: impl ClientInterface) -> anyhow::Result<()> {
        ...
        for protocol in protocols {
            ...
            iface
                .info(&self.common, self.target_sel, protocol, |message| {
                    let is_success =
...
impl Display for DebugPortInfoNode {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(f, "Debug Port: {}", self.dp_info.version)?;
        ...
        if self.dp_info.version == DebugPortVersion::DPv2 {
            ...
            format_jep(f, self.dp_info.designer)?;
        }
                ...
                let ap_node = match ap {
                    ApInfo::MemoryAp {
                        ...
                        let mut ap_root = Tree::new(format!(
                            ...
                            ap_addr
                                .ap_v1()
                                .map_or_else(|_| String::from("Not APv1"), |addr| addr.to_string())
                        ));
